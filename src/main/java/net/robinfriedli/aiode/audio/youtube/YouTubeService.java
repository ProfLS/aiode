...
import net.robinfriedli.aiode.command.commands.playback.PlayCommand;
import net.robinfriedli.aiode.command.commands.playback.QueueCommand;
...
public class YouTubeService extends AbstractShutdownable {
    ...
    @Value("${aiode.tokens.youtube_credentials}")
    private String apiKey;

    public YouTubeService(HibernateComponent hibernateComponent,
                          YouTube youTube,
                          @Value("${aiode.preferences.youtube_api_daily_quota}") int youtubeApiDailyQuota) {
    ...
    public void redirectSpotify(HollowYouTubeVideo youTubeVideo) throws IOException {
        ...
        if (Aiode.get().getGuildPropertyManager().getProperty("audioOnly") == "true") {
